...
RSpec.feature 'Signup', type: :feature do
  ...
  context 'User must fill in a valid password' do
    scenario 'user left password field empty' do
      sign_up(name: 'test', email: "test@test.com", password: '', password_confirmation: '')
    ...
    scenario "user provides a password longer than 10 characters" do
      sign_up(name: 'test', email: "test@test.com", password: "12345678910", password_confirmation: '12345678910')
  ...
  context "User must provide a valid email address" do
    scenario "user left email field empty" do 
      sign_up(name: 'test', email: '', password: "123456789", password_confirmation: '123456789')
    ...
    scenario "user provides an email address that doesn't have 'at sign'" do
      sign_up(name: 'test', email: "testtest.com", password: "123456789", password_confirmation: '123456789')

      expect(page).to have_content("Email is invalid")
    end

    scenario "user provides an email address that doesn't have unique user name" do
      sign_up(name: 'test', email: "@test.com", password: "123456789", password_confirmation: '123456789')

      expect(page).to have_content("Email is invalid")
    end

    scenario "user provides an email address that doesn't have domain name" do
      sign_up(name: 'test', email: "test@", password: "123456789", password_confirmation: '123456789')
